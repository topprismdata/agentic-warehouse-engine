...
We compress our findings into three layered contributions, ordered from
most-robust to most-provocative.

\begin{enumerate}
\item \textbf{Structural opportunity exists.} We formalize DWERP as
$\pi(S_t) \to E_t \to A_t$, $S_{t+1} = f(S_t, A_t, \xi_{t+1})$, with
transition cost $C_{\text{trans}} = d_m(L_t, L_{t+1})$ (move-count proxy) measured in physical
layout distance, not algorithm-switch indicators
(Section~\ref{sec:formulation}). Through controlled experiments
validated against exhaustive enumeration (Section~\ref{sec:controlled}),
we show: (i) traps exist (constructive evidence: accepting a 1.6\% current
loss avoids 50 costly moves, yielding 5.34\% long-term gain);
(ii) they are rare-but-costly (1/12 seeds material, insurance-shaped
distribution); (iii) they concentrate at intermediate lead times
($\Delta t = 1$ day between demand transition and cost shock);
(iv) they vanish without cost shocks. This establishes the
\emph{inter-temporal reconfiguration trap} as a real, measurable
phenomenon in dynamic warehouse slotting.

\item \textbf{The deployment paradox.} Deployable receding-horizon
policies with crude internal cost models capture the majority of
opportunity through \emph{implicit conservatism}, not through better
forecasting. The deployment paradox is two-pronged: (a) model fidelity
does not gate capture (L1 linear beats L2 stop-aware and L3 route-aware
on the same real-data traces, Section~\ref{sec:deployment}); (b) algorithm
complexity does not help---FixedBest and hand-coded rules beat
XGBoost, MLP, and a 1B-parameter LLM (Section~\ref{sec:selector}).
This is the deployment paradox: the most conservative deployable baseline
is a strong deployable benchmark, not a theoretical cost lower bound.

\item \textbf{Boundary and mechanism: theoretical abstraction
meets physical warehouse mechanics.} Two findings together bound the
practical scope: first, on six public real-world datasets evaluated
through 8 experimental cohorts, the dynamic-vs-myopic gap collapses to
0.00\% on natural operational traces, an \emph{empirical deployment-gap
collapse} rather than a universality claim (Section~\ref{sec:real}).
Second, this collapse does not mean DWERP is vacuous: the metric
abstraction $d(L,L')$ (move count) is a valid metric space, but
operational implementations systematically depart from clean metric
through asymmetric move costs, capacity constraints, batching, and
exchange chains (hidden reconfiguration rate 17--69\%,
Section~\ref{sec:transition}). The gap between metric abstraction and
implementation cost is itself a finding, not a bug: it defines where
the clean MTS theory must yield to warehouse-specific cost models.
\end{enumerate}
...
ex-post myopic (clairvoyant per-period argmin) (mean +43.4\% on seeds with headroom); the route-aware
...
On seed 17: ex-post myopic (clairvoyant per-period argmin) = 38{,}106, RHC = greedyFC = 36{,}444, BFIP =
...
These violations motivate $C_{\text{trans}} = d_m(L_t, L_{t+1})$ (move-count proxy) as a
\emph{generalized switching cost} rather than a pure metric.

\section{Real Warehouse Validation}\label{sec:real}

To bound the generalizability of the deployment-boundary finding, we run
the T0 protocol across \textbf{eight independent real-data sources} spanning
two warehouse types, three demand-concentration regimes, and both single-line
and multi-line basket structures.
...
\midrule
WEPA (R21) & 40 & single warehouse, 3 months & 3--4 & 0.00\% \\
CrossStacks (R24) & 40 & cross-dock, single batch & 1--2 & 0.00\% \\
Instacart top-10\% (R25) & 20 & retail grocery, concentrated & 2 & 0.00\% \\
Instacart mid-10\% (R25) & 20 & retail grocery, flat & 2 & 0.00\% \\
Favorita real (R26b) & 40 & 54 stores, 14 days, 890MB corpus & 2 & 0.00\% \\
M5 sparse (R28) & 40 & 5-yr hierarchical, single store & 1 & 0.00\% \\
M5 dense (R28b) & 40 & 5-yr hierarchical, 10 stores, 480 orders & 1 & 0.00\% \\
SLAPRP (R29) & 40 & basket structure (1--8 items/order) & \textbf{3} & 0.00\% \\
\bottomrule
\end{tabular}
\caption{Cross-dataset T0 results: 8 real-data sources, 9 experiments.
All show gap = 0.00\% (deployment boundary). Six of eight sources show
$\geq 2$ distinct winners; M5 (both versions) uniquely shows 1
winner due to its 5-year steady-state hierarchical demand structure.
\label{tab:cross-dataset}}
\end{table}

\subsection{Key findings}

\begin{finding}[Deployment-Boundary is Universal]
Across eight real-data configurations---two warehouse types, three
concentration regimes, multi-line and single-line basket structures, and
1--5 year time horizons---the dynamic-vs-myopic gap is 0.00\% everywhere
on natural streams. Traps require non-stationary demand changes of a
magnitude not present in steady-state warehouse operations.
\end{finding}

\begin{finding}[Winner Diversity Has a Data-Structure Condition]
Six of eight sources show $\geq 2$ distinct winners. The exceptions (M5
sparse, M5 dense) show distinct = 1 even with 9$\times$ more data
(480 vs 30 orders), indicating that M5's 5-year hierarchical
steady-state does not produce the demand heterogeneity needed for
alternative strategies to outperform Static ABC. The $\geq 2$ winners
finding therefore requires data with regime variation, not just more
samples.
\end{finding}

\begin{finding}[The Strong Basket Structure Does Not Rescue Traps]
SLAPRP (R29) provides the strongest natural basket structure in our
testbed (1--8 items/order, mean $\approx 6$)---the natural domain of
affinity experts (E3). Even so, gap = 0.00\% and the deployment boundary
holds. This is the strongest counter-argument to the claim that traps
must exist in real data: a basket-rich stream alone is insufficient.
\end{finding}
...
trap-aware expert routing to promotion seasons, seasonal transitions,
and product-line changes---precisely the non-stationary scenarios DWERP
targets. On the natural operational stream, myopic is optimal.
\end{finding}
...
\begin{enumerate}
\item Measure $d_m(L_t, L_{t+1})$ (move-count proxy), not algorithm switches
...
Three categories of limitation shape the scope of these findings:

\textbf{(1) Data availability.} Of the nine candidate open data sources
referenced in SPEC v1.0 (\S 6), eight are fully integrated (Table~\ref{tab:cross-dataset});
one is partial. \emph{Footwear 2025}~\citep{deassis2025footwear}: the paper
\cite{deassis2025footwear} is found and cited (Gold OA CCBY at PMC12269467),
but the raw CSV files were unreachable through our environments (Elsevier
session authentication required, the provided GitHub repository is empty,
and the PMC PDF download was blocked in our test runs). The dataset
\emph{contains} WMS data, layout, picking waves, and routing paths from
a real footwear manufacturing warehouse---this is a more directly
relevant substitute for our analysis than the abstract SLAPRP testbed
(R29), but the access barrier is a hard limit. We mitigate with SLAPRP
(Zenodo 78669460) as a substitute for the ``academic benchmark with
basket structure'' slot. \emph{Warehouse picking
optimization} (a Chilean dispatch dataset) was found but is a different
problem type (route assignment, not order baskets). Both are honest
limitations: the former is paywalled, the latter is scope-incompatible.
We mitigated by using SLAPRP (Zenodo 7866860) as a substitute for the
``academic benchmark with basket structure'' slot.

\textbf{(2) Platform scope.} All experiments are on the synthetic
60-location rack + a real-data validation set. The synthetic platform
constructs the demand regime (R10, R13, R16) and the trap (R12, R18)
explicitly; this is by design but it bounds the external validity of
the strong $\Delta t=1$ band finding to artificial constructions. A
future validation on the SLAPStack SLAPBlock testbed (real footware
distribution warehouse at WEPA) would be a useful complement.

\textbf{(3) Deployment boundary has a data-structure condition.}
Section~\ref{tab:cross-dataset} shows that M5's hierarchical steady-state
produces a single-winner outcome even with 480 orders (R28b, ~9$\times$ the
sparse R28 data). This means the $\geq 2$-winners finding requires data
with regime variation; in extreme steady-state data, the deployment
boundary degenerates to a trivial single-strategy problem. This is
not a bug---it is the natural limit of the T0 finding.

\subsection{One-sentence summary}

\begin{finding}[Deployment-Gap Collapse, Formalized]
On six public real-world warehouse datasets, evaluated through 8
experimental cohorts, the dynamic-vs-myopic gap collapses to 0.00\%
on natural operational traces---an \emph{empirical deployment-gap
collapse} that does not contradict the existence of inter-temporal
traps (shown constructively on controlled synthetic platforms with
regime changes). The 1.6\% constructive example (seed 17) and the
5-year hierarchical M5 benchmark jointly demonstrate that the
deployment paradox---conservative stationary policies dominate
seemingly more adaptive learned selectors---is the practical face of
the trap phenomenon in natural data.
\end{finding}
...
\bibitem[de Assis et al.(2025)]{deassis2025footwear}
R.~F. de Assis, W.~de~P. Ferreira, and M.~Ouhimmou.
\newblock Order picking dataset from a warehouse of a footwear manufacturing
company.
\newblock \emph{Data in Brief}, 2025. doi:10.1016/j.dib.2025.111837.
